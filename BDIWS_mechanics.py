# =============================================================================
# This file provides the basic image processing mechanics.
...
class WaterMarkCore:
    def __init__(self, password_img=1, robustness_d1=36, robustness_d2=20):
        self.block_shape = np.array([4, 4])
        self.password_img = password_img
        self.d1, self.d2 = robustness_d1, robustness_d2  # d1/d2 越大鲁棒性越强,但输出图片的失真越大
    ...
    def extract_raw(self, img):
        ...
        for channel in range(3):
            wm_block_bit[channel, :] = self.pool.map(self.block_get_wm_fast,
...
class WaterMark:
    def __init__(self, password_wm=1, password_img=1, d1=36, d2=20, block_shape=(4, 4)):

        self.bwm_core = WaterMarkCore(password_img=password_img, robustness_d1=d1, robustness_d2=d2)
    ...
    def read_wm(self, wm_content, mode='img'):
        assert mode in ('img', 'str'), "mode should be 'img' or 'str'!"
        if mode == 'img':
    ...
    def extract(self, filename=None, embed_img=None, wm_shape=None, out_wm_name=None, mode='img'):
        ...
        if mode == 'img':
            ...
            byte = ''.join((np.round(wm)).astype(int).astype(str)) # Fixed a warning
